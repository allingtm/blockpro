...
- **Remedials can be raised on negative answers (optional).** Selecting a negative answer (**No** / **Unsatisfactory**) auto-expands an inline "Add a remedial" form on the question: title, location, description, priority (**Low** / **High**, default Low), and — when the asset has register items — a multi-select of related register items. At most **one** remedial per question, and it is entirely optional: leaving the title blank means no remedial is raised. Changing the answer back to a positive one **discards** the remedial (same policy as photo-clearing). The remedial flows through drafts and the offline outbox, and is sent per-answer on `app_completed-inspection` (see BlockPro-API-Spec-v2.md §5.10). Existing remedials from past inspections remain read-only context.
